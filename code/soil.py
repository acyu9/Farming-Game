...
class Plant(pygame.sprite.Sprite):
    def __init__(self, plant_type, groups, soil, check_watered):
        ...
        # Accommodates different plant having different size
...
class SoilLayer:
    # Attach soil patches to all sprites
    ...
    def create_soil_grid(self):
        # Don't need to convert b/c not showing to player
        ground = pygame.image.load('./graphics/world/ground.png')
        h_tiles = ground.get_width() // TILE_SIZE
        v_tiles = ground.get_height() // TILE_SIZE

        # List for each row and list for each cell
        self.grid = [[[] for col in range(h_tiles)] for row in range(v_tiles)]
        
        for x, y, _ in load_pygame('./data/map.tmx').get_layer_by_name('Farmable').tiles():
            # If the area is farmable, F
            # [row][column]
    ...
    def get_hit(self, point):
        # Point the player can hit
        for rect in self.hit_rects:
            if rect.collidepoint(point):
                self.hoe_sound.play()

                # Convert rect back into grid
                x = rect.x // TILE_SIZE
                y = rect.y // TILE_SIZE
            
                if 'F' in self.grid[y][x]:
                    # Soil patch on this tile
        ...
        for index_row, row in enumerate(self.grid):
            for index_col, cell in enumerate(row):
                if 'X' in cell and 'W' not in cell:
                    # Find soil tile that's not watered yet
                    cell.append('W')
                    x = index_col * TILE_SIZE
                    y = index_row * TILE_SIZE
                    WaterTile(
    ...
    def create_soil_tiles(self):
        # Empty soil sprites then add all the soils together to make them look nicer/ connected
        self.soil_sprites.empty()
        for index_row, row in enumerate(self.grid):
            for index_col, cell in enumerate(row):
                if 'X' in cell:
                    # Tile options
                    # Find top, right, left, bottom around the tile
                    t = 'X' in self.grid[index_row - 1][index_col]
                    b = 'X' in self.grid[index_row + 1][index_col]
                    r = 'X' in self.grid[index_row][index_col + 1]
                    l = 'X' in self.grid[index_row][index_col - 1]

                    # Default tile is o (center soil patch)
                    tile_type = 'o'
